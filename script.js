// ===== Validasi Formulir Transaksi dengan JavaScript =====
document.addEventListener("DOMContentLoaded", function () {
  ...
  form.addEventListener("submit", function (e) {
    ...
    // Ambil semua input, select, textarea wajib di dalam form
    ...
    fields.forEach((field) => {
      ...
      if (field.value.trim() === "") {
        ...
        if (errorSpan) {
          errorSpan.textContent = "Kolom ini wajib diisi.";
    ...
    // Validasi tambahan: jumlah harus angka positif
    ...
    if (jumlahField && jumlahField.value && Number(jumlahField.value) <= 0) {
      ...
      if (errorSpan) {
        errorSpan.textContent = "Jumlah harus lebih dari 0.";
        errorSpan.style.display = "block";
      }
    }

    if (!isValid) {
      formMessage.textContent = "Gagal menyimpan! Mohon lengkapi semua data dengan benar.";
      formMessage.style.color = "#e74c3c";
      return;
    }

    // Jika valid -> simpan ke localStorage agar muncul di Daftar Transaksi
...
// ===== Menampilkan Daftar Transaksi (halaman daftar-transaksi.html) =====
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  loginForm.addEventListener("submit", function (e) {
    ...
    loginMessage.textContent = "Login berhasil! Selamat datang, " + username + ".";
